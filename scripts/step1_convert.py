#!/usr/bin/env python3
# step1_convert.py -- GGUF bf16 -> HF safetensors (LLM + optional Vision)
# Streaming approach: process tensors in batches, save shards incrementally
import argparse
import gc
import json
import os
...
def build_llm_mapping(num_layers: int, full_attn_layers: set):
    mapping = {}
...
def build_vision_mapping(num_vision_blocks: int):
    mapping = {}
...
# ---------------------------------------------------------------------------
# Tensor conversion (includes 3 critical fixes)
# ---------------------------------------------------------------------------
...
def gguf_tensor_to_torch(tensor_info, hf_name=""):
    ...
    # FIX 1: GGUF stores RMSNorm weights as (1 + learned_param), HF expects learned_param
    ...
    # FIX 2: GGUF stores A as -exp(A_log), HF expects A_log
    if gguf_name.endswith(".ssm_a"):
        t = (-t.float()).log().to(torch.bfloat16)

    # FIX 3: GGUF (llama.cpp) stores value heads in (3,16) grouping order,
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert GGUF bf16 model(s) to HF safetensors format"
    )
    parser.add_argument("--gguf-llm", required=True,
                        help="Path to LLM GGUF file")
    parser.add_argument("--gguf-vision", default=None,
                        help="Path to vision/mmproj GGUF file (optional)")
    parser.add_argument("--output-dir", required=True,
                        help="Output directory for HF safetensors")
    parser.add_argument("--reference-repo",
                        default="huihui-ai/Huihui-Qwen3.5-27B-abliterated",
                        help="HF repo for config/tokenizer files")
    ...
    parser.add_argument("--num-vision-blocks", type=int, default=27,
                        help="Number of vision transformer blocks")
    parser.add_argument("--full-attn-interval", type=int, default=4,
                        help="Full attention every N layers")
    parser.add_argument("--full-attn-start", type=int, default=3,
                        help="First full attention layer index")
    args = parser.parse_args()

    shard_size_bytes = parse_shard_size(args.shard_size)
    full_attn_layers = set(range(args.full_attn_start, args.num_layers, args.full_attn_interval))
    output_dir = args.output_dir

    os.makedirs(output_dir, exist_ok=True)

    # --- Download config/tokenizer from reference repo ---
    ...
    for fname in config_files:
        ...
        if os.path.exists(dst):
            ...
            continue
        try:
            path = hf_hub_download(args.reference_repo, fname)
    ...
    # --- Build mappings ---
    llm_mapping = build_llm_mapping(args.num_layers, full_attn_layers)

    writer = ShardWriter(output_dir, shard_size_bytes)

    # --- Process LLM ---
    process_gguf(args.gguf_llm, llm_mapping, "LLM", writer)

    # --- Process Vision (optional) ---
    if args.gguf_vision:
        vision_mapping = build_vision_mapping(args.num_vision_blocks)
        process_gguf(args.gguf_vision, vision_mapping, "Vision", writer)
    ...
    idx_path = hf_hub_download(args.reference_repo, "model.safetensors.index.json")
